...
export class AppService {
  constructor(
    ...
    private appService5: AppService5,
  ...
  async getHello() {
    ...
    for (let j = 0; j < 100; j++) {
      for (let index = 0; index < balanceIdList.length; index++) {
        ...
        // setTimeout(() => {
        i++
        await this.testQueue.add(`test111`, { balanceId, amount, i, rawData }, jobOpts);
        await this.testQueue2.add(`test111`, { balanceId, amount, i, rawData }, jobOpts);
        await this.testQueue3.add(`test111`, { balanceId, amount, i, rawData }, jobOpts);
        await this.testQueue4.add(`test111`, { balanceId, amount, i, rawData }, jobOpts);
        await this.testQueue5.add(`test111`, { balanceId, amount, i, rawData }, jobOpts);
        // await this.testQueue6.add(`test111`, { balanceId, amount, index, rawData }, jobOpts);
        // await this.testQueue7.add(`test111`, { balanceId, amount, index, rawData }, jobOpts);
